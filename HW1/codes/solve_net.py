...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq, run_wandb=False):
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            ...
            if run_wandb: wandb.log({"train/loss": mean_loss, "train/accuracy": mean_acc})
...
def test_net(model, loss, inputs, labels, batch_size, run_wandb=False):
    loss_list = []
    acc_list = []
    ...
    if run_wandb: wandb.log({"test/loss": mean_loss, "test/accuracy": mean_acc})
